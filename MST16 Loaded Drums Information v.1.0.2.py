...
"""
Releases history:
MST16 Loaded Drums Information v.1.0.2:
added MS SQL and MS ACCESS queries separation
"""
program_name = "MST16 Loaded Drums Information v.1.0.2"
about_program_text = [program_name + "\nThe program is designed to work on MST16 of Ignalina Nuclear Power Plant.\n"
                      "It shows information about drums loaded into a container on MST16.\n"
                      "The program connects to Tracking database, collects and represents information for present on MST16 container or for any other container (for entered barcode).\n\n"
                      "Used programming language: Python.\n\n"
                      "Developer: Jevgenij Kariagin\n"
                      "email: mahoni1983@mail.ru\n"
                      "2020-04",
                      program_name + "\nПрограмма предназначена для работы на MST16 Игналинской атомной электростанции.\n"
                      "Показывает информацию о бочках, загруженных в контейнер.\n"
                      "Программа подсоединяется к базе данных Tracking комплекса по переработке радиоактивных отходов, собирает и показывает информацию по установленному на MST16 контейнеру или по другому контейнеру (по введённому штрих-коду).\n"
...
query_current_Access = "select q.ContID_ContPressed as Container, q.ID_Drums as Drum, w.ColID_ContPressed&w.RowID_ContPressed as Position, w.StateDate_ContPressed as Loaded_date" \
                ", q.Waste_Drums as Waste, q.Vectorname as NV, q.Height_Drums as Height, q.StartDate_MST6DMS as Measured_date" \
               ", q.MassBrutto_MST6DMS as Mass_brutto, q.MassNetto_MST6DMS as Mass_netto, q.DRSMean_MST6DMS as DRM_mean, q.DRSMax_MST6DMS as DRM_max" \
               ", q.DR1mMax_MST6DMS as DRM1m_max, q.Co60Acttivity as Co60_Bq, q.Co60ActivitySigma as Co60Unc_Bq" \
               ", q.Co60Acttivity/q.MassNetto_MST6DMS as Co60_Bq_kg " \
               "from UserView_TST16_DrumsDetailed as q " \
               "inner join " \
               "(select * from T_ContPressed ) as w " \
               "on  " \
               "q.ID_Drums = w.DrumID_ContPressed " \
               "where q.ContID_ContPressed in (select ID_Cont from T_Cont where lastAction_Cont = 1601600) " \
               "order by w.ID_ContPressed"
                #"where q.ContID_ContPressed = 100000000651 " \
#in (select ID_Cont from T_Cont where lastAction_Cont = 700700) "

# query to get info for btn_current for MS SQL
query_current_MsSql = "select q.ContID_ContPressed as Container, q.ID_Drums as Drum, STR(w.ColID_ContPressed) + w.RowID_ContPressed as Position, w.StateDate_ContPressed as Loaded_date" \
                ", q.Waste_Drums as Waste, q.Vectorname as NV, q.Height_Drums as Height, q.StartDate_MST6DMS as Measured_date" \
               ", q.MassBrutto_MST6DMS as Mass_brutto, q.MassNetto_MST6DMS as Mass_netto, q.DRSMean_MST6DMS as DRM_mean, q.DRSMax_MST6DMS as DRM_max" \
               ", q.DR1mMax_MST6DMS as DRM1m_max, q.Co60Acttivity as Co60_Bq, q.Co60ActivitySigma as Co60Unc_Bq" \
               ", q.Co60Acttivity/q.MassNetto_MST6DMS as Co60_Bq_kg " \
...
query_custom_Access = "select q.ContID_ContPressed as Container, q.ID_Drums as Drum, w.ColID_ContPressed&w.RowID_ContPressed as Position, w.StateDate_ContPressed as Loaded_date" \
                ", q.Waste_Drums as Waste, q.Vectorname as NV, q.Height_Drums as Height, q.StartDate_MST6DMS as Measured_date" \
               ", q.MassBrutto_MST6DMS as Mass_brutto, q.MassNetto_MST6DMS as Mass_netto, q.DRSMean_MST6DMS as DRM_mean, q.DRSMax_MST6DMS as DRM_max" \
               ", q.DR1mMax_MST6DMS as DRM1m_max, q.Co60Acttivity as Co60_Bq, q.Co60ActivitySigma as Co60Unc_Bq" \
               ", q.Co60Acttivity/q.MassNetto_MST6DMS as Co60_Bq_kg " \
               ...
               "on  " \
               "q.ID_Drums = w.DrumID_ContPressed " \
               "where q.ContID_ContPressed = $to_replace$ " \
               "order by w.ID_ContPressed"

# query to get info for btn_custom
query_custom_MsSql = "select q.ContID_ContPressed as Container, q.ID_Drums as Drum, STR(w.ColID_ContPressed) + w.RowID_ContPressed as Position, w.StateDate_ContPressed as Loaded_date" \
                ", q.Waste_Drums as Waste, q.Vectorname as NV, q.Height_Drums as Height, q.StartDate_MST6DMS as Measured_date" \
               ", q.MassBrutto_MST6DMS as Mass_brutto, q.MassNetto_MST6DMS as Mass_netto, q.DRSMean_MST6DMS as DRM_mean, q.DRSMax_MST6DMS as DRM_max" \
               ", q.DR1mMax_MST6DMS as DRM1m_max, q.Co60Acttivity as Co60_Bq, q.Co60ActivitySigma as Co60Unc_Bq" \
               ", q.Co60Acttivity/q.MassNetto_MST6DMS as Co60_Bq_kg " \
...
list_columns = [
    ["Container", "Drum", "Position", "Loaded_date", "Waste", "NV", "Height", "Measured_date", "Mass_brutto", "Mass_netto", "DRM_mean", "DRM_max", "DRM1m_max", "Co60_Bq", "Co60Unc_Bq", "Co60_Bq_kg"],
    ["Container", "Drum", "Position", "Loaded date", "Waste", "Nuclide Vector", "Height,\n mm", "Measurement date", "Mass\n brutto,\n kg", "Mass\n netto,\n kg", "DRM mean,\n mSv/h", "DRM max,\n mSv/h", "DRM1m max,\n mSv/h", "Co60, Bq", "Co60Unc,\n Bq", "Co60,\n Bq/kg"],
    ["Контейнер", "Бочка", "Позиция", "Дата загрузки", "Отходы", "Нуклидный вектор", "Высота,\n мм", "Дата измерения", "Масса\n брутто,\n кг", "Масса\n нетто,\n кг", "МЭД ср.,\n мЗв/ч", "МЭД макс.,\n мЗв/ч", "МЭД 1м,\n мЗв/ч", "Co60, Бк", "Co60 неопр.,\n Бк", "Co60,\n Бк/кг"]
]

dict_columns_to_summ = {"Drum": 'count', "Position": 'each count', "Loaded_date": 'range date', "Waste": 'each count', "NV": 'each count', "Height": 'sum',
...
list_columns_to_format = [
    ["Height", "Mass_brutto", "Mass_netto", "DRM_mean", "DRM_max", "DRM1m_max", "Co60_Bq", "Co60Unc_Bq", "Co60_Bq_kg"],
...
dict_controls = {'btn_about': ["About program", "О программе"],
                 ...
                 'lbl_current': ["Current container (on MST16)", "Контейнер на MST16"],
...
def show_info_about_current_container():
    ...
    if connector == None:  # failed to connect to DB
        ...
        if b_connected_to_MS_Access:
            cursor.execute(query_current_Access)
        else:
            cursor.execute(query_current_MsSql)
